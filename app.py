...
if "action" in query_params and query_params["action"] == "salvar_obs":
    ...
    if "Aluno" in df_sheet.columns and "Observações" in df_sheet.columns:
        df_sheet.loc[
            df_sheet["Aluno"].astype(str).str.strip().str.upper()
            == aluno_alvo.strip().upper(),
            "Observações"
        ] = nova_obs
        ...
        st.toast(
            f"Deliberação de {aluno_alvo} salva com sucesso no Google Sheets!",
            icon="✅"
        )
...
def extrairDados(arquivosPdf):
    ...
    for arquivo in arquivosPdf:
        ...
        blocosBoletins = re.split(
            r"(?=BOLETIM DE NOTAS INDIVIDUAL|Aluno\(a\):)",
            textoCompleto
        )
        ...
        for bloco in blocosBoletins:
            ...
            # Mantém 100 como valor padrão caso a frequência não seja encontrada
            freqGlobal = 100.0
            ...
            if not nomeAluno:
                ...
                mNome2 = re.search(
                    r"BOLETIM DE NOTAS INDIVIDUAL\s*([^\n]+)",
                    bloco,
                    re.IGNORECASE
                )
            ...
            # FREQUÊNCIA
            # ---------------------------------------------------------
            # A frequência não é mais procurada somente pelo padrão
            # "Frequência: XX%".
            #
            # Ela será identificada também dentro dos blocos das
            # disciplinas, como no segundo código.
            # ---------------------------------------------------------
            ...
            # (versão flexível com e sem dois-pontos)
            necEspeciais = "Não"
            tipoNecEspecial = "-"
            transtorno = "Não"
            tipoTranstorno = "-"
            ...
            for match in padraoBloco:
                ...
                if (
                    "Turma:" in nomeMateria
                    or "Sit." in nomeMateria
                    or "Série" in nomeMateria
                ):
                    continue
                ...
                # -----------------------------------------------------
                # Igual à lógica utilizada no segundo código:
                # números de 10 a 100 encontrados no bloco da
                # disciplina podem representar a frequência.
                # -----------------------------------------------------

                candidatosFrequencia = re.findall(
                    r"\b(\d{1,3}[\.,]?\d{0,2})\b",
                    blocoTexto
                )

                for val in candidatosFrequencia:
                ...
                b2 = (
                    notasEncontradas[1]
                    if len(notasEncontradas) > 1
                    else None
                )

                b3 = (
                    notasEncontradas[2]
                    if len(notasEncontradas) > 2
                    else None
                )

                b4 = (
                    notasEncontradas[3]
                    if len(notasEncontradas) > 3
                    else None
                )


                notasValidas = [
                    n for n in [b1, b2, b3, b4]
                    if n is not None
                ]

                mediaFinal = (
                    round(sum(notasValidas) / len(notasValidas), 2)
                    if notasValidas
                    else 0.0
                )
...
if not st.session_state.dadosCarregados:
    ...
    st.subheader(
        "Selecione a sala e faça o upload dos relatórios em PDF."
    )
    ...
    if st.button("PROCESSAR E ATUALIZAR DASHBOARD"):
        ...
        if arquivosEnviados:
            ...
            with st.spinner(
                "Processando arquivos, atualizando planilha e gerando JSON..."
            ):
                ...
                if "Aluno" in df_atual.columns:
                    ...
                    df_atual = df_atual[
                        ~df_atual["Aluno"]
                        .astype(str)
                        .str.contains("BT30", na=False)
                    ]

                    df_atual = df_atual[
                        df_atual["Aluno"] != "Não Identificado"
                    ]
                ...
                if not BDNovo.empty:
                    ...
                    dicionario_dados = df_final.to_dict(
                        orient="records"
                    )
                    ...
                    st.error(
                        "Não foi possível extrair dados válidos do PDF."
                    )
...
# ============================================================
    ...
    st.sidebar.write(
        f"Visualizando: **{st.session_state.salaAtiva}**"
    )
    ...
    if os.path.exists("dados_alunos.json"):
        ...
        linkSalaAtiva = DICIONARIO_SALAS[
            st.session_state.salaAtiva
        ]
        ...
        dados_para_html = (
            df_sheet
            .where(pd.notnull(df_sheet), None)
    ...
    for item in dados_para_html:
        ...
        if prontuario not in alunosMapeados:
            ...
            if (
                str(item.get("Necessidades Especiais"))
                .strip()
                .lower()
                == "sim"
            ):
                ...
                pneTexto.append(
                    f"PNE: {item.get('Tipo de Necessidade Especial') or '-'}"
                )


            if (
                str(item.get("Transtorno"))
                .strip()
                .lower()
                == "sim"
            ):
                ...
                pneTexto.append(
                    f"Transtorno: {item.get('Tipo de Transtorno') or '-'}"
                )


            if (
                str(item.get("Superdotação"))
                .strip()
                .lower()
                == "sim"
            ):
                ...
                pneTexto.append(
                    f"Superdotação: {item.get('Tipo de Superdotação') or '-'}"
                )
            ...
            infoNapne = (
                ...
                "Nenhum registro de PNE/Transtorno/Superdotação."
            )
            ...
            alunosMapeados[prontuario] = {
                ...
                "deliberacao": (
                    str(item.get("Observações", ""))
                    if item.get("Observações")
                    and str(item.get("Observações")).lower() != "nan"
                    else ""
                ),
    ...
    json_estruturado = json.dumps(
        list(alunosMapeados.values()),
        ensure_ascii=False
    )
    ...
    if os.path.exists("index.html"):
        ...
        html_injetado = html_content.replace(
            "__DADOS_JSON_INJETADOS__",
            json_estruturado
        )


        b64_html = base64.b64encode(
            html_injetado.encode("utf-8")
        ).decode("utf-8")
